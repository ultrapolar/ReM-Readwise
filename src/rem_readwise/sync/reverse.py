...
class ReverseSync:
    def __init__(
        ...
        *,
        ...
        self._state = state
        # The sync folder plus any extras (e.g. the Done folder, so highlights
        # on finished docs are pulled before the finish pass archives them).
    ...
    def run(self, documents: list[ReaderDocument] | None = None) -> ReverseResult:
        ...
        for folder in self._folders:
            for entry in self._remarkable.list_folder(folder):
                if entry.is_dir:
                    continue
                resolution = resolve_reader_id(
                    entry.name, folder, self._remarkable, self._state, dry_run=self._dry_run
                )
                ...
                # Backfill the stable device id for docs uploaded before ids
                # were tracked, so a future rename of them is survivable too.
                # One stat per doc, once ever.
                if self._state.device_id_for(reader_id) is None and not self._dry_run:
                    device_id = self._remarkable.stat(f"{folder}/{entry.name}")
                    if device_id:
                ...
                try:
                    self._process_document(entry.name, reader_id, folder, download_dir, result)
    ...
    def _process_document(
        ...
        remote_path = f"{folder}/{remarkable_name}"
